    ...
    for col in df.columns:
        ...
        new_parts = [
            datetime.datetime.utcfromtimestamp(
                float(part)/1000).strftime(date_format)
...
def df_to_excel(df: pd.DataFrame, sheet_name='Sheet1', from_report=False, slice: Union[Slice, None] = None,
    ...
    if is_pivot(slice):
        ...
        if groupbyRows:
            ...
            for i, groupby in enumerate(groupbyRows):
                rename_dict[f'level_{i}'] = get_column_names(
                    [groupby], verbose_map)[0]
            ...
            if 'index' in df.columns:
                rename_dict['index'] = get_column_names(
                    [groupbyRows[0]], verbose_map)[0]
        ...
        if bool(slice.form_data.get("rowTotals")):
            ...
            df.rename(
                columns={last_col_name: translated_col_name}, inplace=True)
            ...
            if column_config:
                for idx, column in enumerate(df.columns):
                    ...
                    if not in_column_config:
                        ...
                        if verbose_map:
                            verbosed_name = next(
                                (key for key, value in verbose_map.items() if value == col_name), None)
    ...
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        ...
        formatter = ExcelColumnFormatter(workbook)
        ...
        for col_num, value in enumerate(df.columns.values):  # manzana custom
            ...
            if slice:
                if is_pivot(slice):
                    valueFormat = slice.form_data.get('valueFormat')
                    if valueFormat:
            ...
            if value in df.columns:
                ...
                if isinstance(column_data, pd.Series) and column_data.dtype == 'object':
                    ...
                    try:
                        ...
                        valid_dates_converted = pd.to_datetime(
                            valid_dates, format=date_format_from_dict, errors='coerce')

                        # Handle or replace NaT values after conversion if necessary
                        # Consider what you want to do with NaT values
                        valid_dates_converted.fillna(
                            "YourPlaceholderOrAction", inplace=True)
        ...
        if (from_report):
            try:
                ...
                if (date_format_key):
                    date_format = request_to_excel_format.get(
                        date_format_key, 'dd/mm/yyyy')
                    format_unixtimestamp_in_colname(
                        df, date_format, workbook, worksheet, header_format)  # type: ignore
            except Exception as err:
        ...
        if bool(slice and slice.form_data.get("colTotals")):
